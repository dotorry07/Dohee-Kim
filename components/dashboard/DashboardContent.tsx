...
import { academicEvents, personalTodaySchedules, todayMeal } from "@/data/dashboardMock";
import { notices, posts, timetables } from "@/lib/data";
import { getTodayClasses } from "@/lib/timetable";
import type { DashboardIconName, DashboardUser, ScheduleType, TodayScheduleItem } from "@/types/dashboard";
  ...
  return <main className={styles.page}><div className={styles.container}>
    ...
    <section className={styles.twoGrid}>
      <article className={styles.card}><Heading icon="calendar" title="오늘의 일정" meta={`${todaySchedules.length}개`}/>
        {todaySchedules.length ? <><ol className={styles.scheduleList}>{todaySchedules.map((item) => <ScheduleItem item={item} key={item.id} state={item.id === currentSchedule?.id ? "current" : item.id === nextSchedule?.id ? "next" : undefined}/>)}</ol><div className={styles.actionArea}><button className={styles.disabledCta} type="button" disabled aria-label="캠퍼스 길찾기, 추후 일정 장소와 연동 예정">캠퍼스 길찾기</button><small>일정 장소 연동 예정</small></div></> : <div className={styles.empty}><p>오늘 예정된 일정이 없습니다.</p></div>}
      </article>
    ...
    <section className={styles.mainGrid}>
      <div className={styles.sideStack}>
        <article className={styles.card}><Heading icon="user" title="학생 프로필"/><div className={styles.profileTop}><div className={styles.avatar} aria-label="기본 프로필 아바타"><Icon name="user"/></div><div><h3>{displayName}</h3><span className={styles.freshmanBadge}>신입생</span></div></div><dl className={styles.profileList}><div><dt>학번</dt><dd>{studentNumber}</dd></div><div><dt>소속</dt><dd>{user.department}</dd></div><div><dt>학년</dt><dd>{user.grade}학년</dd></div><div><dt>이메일</dt><dd title={user.email}>{user.email}</dd></div></dl><button className={styles.disabledButton} type="button" disabled title="프로필 관리 경로 준비 중">내 정보 관리</button></article>
      </div>
    ...
    <section className={styles.bottomGrid}>
      <article className={styles.card}><Heading icon="meal" title="오늘의 학식"/>{todayMeal ? <div className={styles.meal}><div className={styles.mealTitle}><h3>{todayMeal.cafeteria}</h3><span className={styles.mealStatus}>{todayMeal.status === "AVAILABLE" ? "식사 가능" : todayMeal.status === "CLOSING_SOON" ? "마감 임박" : "운영 종료"}</span></div><ul>{todayMeal.menus.map((menu) => <li key={menu}>{menu}</li>)}</ul><div><span>{todayMeal.hours}</span><strong>{todayMeal.price}</strong></div><div className={styles.actionArea}><button className={styles.disabledCta} type="button" disabled aria-label="학식 자세히보기, 주간 학식표 연동 예정">학식 자세히보기</button><small>주간 학식표 연동 예정</small></div></div> : <div className={styles.empty}>오늘 등록된 메뉴가 없습니다.</div>}</article>
      <FreshmanChecklist userId={user.id}/>
    </section>
  </div></main>;
}
